...
var ItemSparqlService = class _ItemSparqlService {
  ...
  itemSparql(item) {
    ...
    this.Q12Test = this.sparqlAsk(this.subclassTest(item.id, "Q12")).pipe(startWith(false));
    ...
    this.sparql2$ = forkJoin([this.Q140759Test, this.Q8Test, this.Q16200Test]).pipe(switchMap(([test1, test2, test3]) => this.selectSparql2(test1, test2, test3, item)), startWith([void 0, void 0]));
    ...
    this.sparql4$ = forkJoin([this.Q8Test]).pipe(switchMap(([test1]) => this.selectSparql4(test1, item)), startWith([void 0, void 0]));
  ...
  selectSparql2(test1, test2, test3, item) {
    let result;
    console.log(test3);
    if (test1 === true) {
      ...
      if (test2 === true) {
        ...
        if (test3 === true) {
          console.log(test3);
          console.log(item.claims.P48[0].mainsnak.datavalue.value);
...
var DetailsService = class _DetailsService {
  ...
  getReferenceProperties(u) {
    ...
    for (const val of values) {
      for (const u2 of val) {
        if (u2.references === void 0) {
          continue;
        }
        for (const ref of u2.references) {
  ...
  setPropertiesList(u) {
    ...
    for (const val of values) {
      for (const u2 of val)
        properties.push(u2.mainsnak.property);
    }
    for (const val of values) {
      for (const u2 of val)
        qualifierProperties = qualifierProperties.concat(u2["qualifiers-order"]);
    }
    for (const val of values) {
      for (const u2 of val) {
        if (u2.references === void 0) {
    ...
    referenceProperties = this.uniq(referenceProperties);
    properties = properties.concat(qualifierProperties).concat(referenceProperties);
    properties = this.uniq(properties);
    properties = this.lessThanFifty(properties);
    let propertiesList0 = this.stringFromArray(properties[0]).slice(1);
    let propertiesList1 = this.stringFromArray(properties[1]).slice(1);
    let propertiesList2 = this.stringFromArray(properties[2]).slice(1);
    let propertiesList3 = this.stringFromArray(properties[3]).slice(1);
    let propertiesList4 = this.stringFromArray(properties[4]).slice(1);
    let propertiesList5 = this.stringFromArray(properties[5]).slice(1);
    let propertiesList6 = this.stringFromArray(properties[6]).slice(1);
    let propertiesList7 = this.stringFromArray(properties[7]).slice(1);
    if (properties[8] !== void 0) {
      alert("too many statements");
    }
    ;
    let result = this.requestService.requestProperties(propertiesList0, propertiesList1, propertiesList2, propertiesList3, propertiesList4, propertiesList5, propertiesList6, propertiesList7).pipe(map((res) => this.mergeObjects(res)), map((res) => Object.values(res)));
    return result;
  }
  setItemsList(u) {
    let values = Object.values(u.claims);
    ...
    for (const val of values) {
      for (const u2 of val) {
        if (u2.mainsnak.datavalue.value.id === void 0)
          continue;
        items.push(u2.mainsnak.datavalue.value.id);
      }
    }
    for (const val of values) {
      for (const u2 of val) {
    ...
    qualifierProperties = this.uniq(qualifierProperties);
    referenceProperties = this.uniq(referenceProperties);
    let qualifierItems = this.setQualifierItems(values, qualifierProperties).filter(function(element) {
      return element !== void 0;
    });
    let referenceItems = this.setReferenceItems(values, referenceProperties).filter(function(element) {
      return element !== void 0;
    });
    items = items.concat(qualifierItems).concat(referenceItems);
    items = this.uniq(items);
    items = this.lessThanFifty(items);
    let itemsList0 = this.stringFromArray(items[0]).slice(1);
    let itemsList1 = this.stringFromArray(items[1]).slice(1);
    let itemsList2 = this.stringFromArray(items[2]).slice(1);
    let itemsList3 = this.stringFromArray(items[3]).slice(1);
    let itemsList4 = this.stringFromArray(items[4]).slice(1);
    let itemsList5 = this.stringFromArray(items[5]).slice(1);
    let itemsList6 = this.stringFromArray(items[6]).slice(1);
    let itemsList7 = this.stringFromArray(items[7]).slice(1);
    if (items[8] !== void 0) {
      alert("too many statements");
    }
    ;
    let result = this.requestService.requestItems(itemsList0, itemsList1, itemsList2, itemsList3, itemsList4, itemsList5, itemsList6, itemsList7).pipe(map((res) => this.mergeObjects(res)), map((res) => Object.values(res)));
    return result;
  }
  setProperties(arr) {
    let result = [];
  ...
  mergeObjects(res) {
    ...
    for (let i = 0; i < res.length; i++) {
      if (res[i].entities === void 0) {
        continue;
      }
      u = __spreadValues(__spreadValues({}, u), res[i].entities);
    }
    return u;
  }
    ...
    for (const val of values) {
      for (const u of val) {
        for (let j = 0; j < arr.length; j++) {
          if (u.qualifiers === void 0) {
            continue;
          }
          if (u.qualifiers[arr[j]] !== void 0) {
            for (let k = 0; k < arr[j].length; k++) {
              if (u.qualifiers[arr[j]][k] === void 0) {
                continue;
              }
  ...
  setReferenceItems(values, arr) {
    ...
    for (const val of values) {
      for (const u of val) {
        if (u.references === void 0) {
          continue;
        }
        for (let i = 0; i < u.references.length; i++) {
          for (let j = 0; j < arr.length; j++) {
            if (u.references[i] !== void 0) {
              if (u.references[i].snaks[arr[j]] !== void 0) {
                if (u.references[i].snaks[arr[j]].datatype = "wikibase-item")
                  for (let k = 0; k < arr[j].length; k++) {
                    if (u.references[i].snaks[arr[j]][k] !== void 0) {
...
var JoinPipe = class _JoinPipe {
  ...
  static {
    this.\u0275pipe = /* @__PURE__ */ \u0275\u0275definePipe({ name: "join", type: _JoinPipe, pure: true });
  }
};
...
var UnitPipe = class _UnitPipe {
  ...
  static {
    this.\u0275pipe = /* @__PURE__ */ \u0275\u0275definePipe({ name: "unit", type: _UnitPipe, pure: true });
  }
};

// src/app/display/item-info/item-info.component.ts
var _forTrack0 = ($index, $item) => $item.label;
var _forTrack1 = ($index, $item) => $item.itemLabel;
var _forTrack2 = ($index, $item) => $item.mainsnak;
...
function ItemInfoComponent_For_6_For_5_Conditional_13_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span");
    \u0275\u0275repeaterCreate(1, ItemInfoComponent_For_6_For_5_Conditional_13_For_2_Template, 6, 1, "div", null, _forTrack0);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const M_r1 = \u0275\u0275nextContext().$implicit;
...
function ItemInfoComponent_For_6_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(3);
    \u0275\u0275elementEnd()();
    \u0275\u0275repeaterCreate(4, ItemInfoComponent_For_6_For_5_Template, 15, 11, "div", 9, _forTrack2);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
...
var ItemInfoComponent = class _ItemInfoComponent {
  constructor() {
    this.lang = inject(SelectedLangService);
    this.instancesList = input();
    this.subclassesList = input();
    this.natureOfList = input();
    this.classesList = input();
    this.technicalities = input();
    this.selectedLang = localStorage["selectedLang"] === void 0 ? "en" : localStorage["selectedLang"];
    this.list1 = [];
    this.list2 = [];
    this.list3 = [];
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ItemInfoComponent, { className: "ItemInfoComponent", filePath: "src/app/display/item-info/item-info.component.ts", lineNumber: 19 });
})();

// src/app/display/header-display/header-display.component.ts
var _forTrack02 = ($index, $item) => $item.label;
var _forTrack12 = ($index, $item) => $item.mainsnak;
var _c02 = (a0) => ["/item", a0];
...
function HeaderDisplayComponent_For_2_For_4_Conditional_3_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span");
    \u0275\u0275repeaterCreate(1, HeaderDisplayComponent_For_2_For_4_Conditional_3_For_2_Template, 6, 1, "div", null, _forTrack02);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const M_r1 = \u0275\u0275nextContext().$implicit;
...
function HeaderDisplayComponent_For_2_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(2);
    \u0275\u0275elementEnd()();
    \u0275\u0275repeaterCreate(3, HeaderDisplayComponent_For_2_For_4_Template, 5, 3, "div", 3, _forTrack12);
  }
  if (rf & 2) {
...
var HeaderDisplayComponent = class _HeaderDisplayComponent {
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _HeaderDisplayComponent, selectors: [["app-header-display"]], inputs: { headerDetail: [1, "headerDetail"] }, decls: 3, vars: 0, consts: [[1, "cardBackground"], [1, "labelTitlePadding"], [1, "propertyStyle"], [1, "statementStyle"], [1, "qualifierStyle"], [1, "internalLink", 3, "routerLink"], ["color", "primary", 3, "inline"], [1, "qualifierProperty"], [1, "details3"], [1, "calendarStyle"], ["target", "_blank", 3, "href"], [1, "displayLayout"], [1, "description-text"], [1, ""], [3, "src"], [1, "referenceStyle"], [1, "inlineStyle"], [1, "referenceProperty"]], template: function HeaderDisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275elementStart(0, "mat-card-content", 0);
        \u0275\u0275repeaterCreate(1, HeaderDisplayComponent_For_2_Template, 5, 1, null, null, _forTrack02);
        \u0275\u0275elementEnd();
      }
      if (rf & 2) {
        \u0275\u0275advance();
    ...
    }, dependencies: [CommonModule, AsyncPipe, MatIconModule, MatIcon, MatCardModule, MatCardContent, RouterLink, UnitPipe], encapsulation: 2 });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(HeaderDisplayComponent, { className: "HeaderDisplayComponent", filePath: "src/app/display/header-display/header-display.component.ts", lineNumber: 15 });
})();

// src/app/display/career-display/career-display.component.ts
var _forTrack03 = ($index, $item) => $item.label;
var _forTrack13 = ($index, $item) => $item.mainsnak;
...
function CareerDisplayComponent_For_4_For_5_Conditional_13_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span");
    \u0275\u0275repeaterCreate(1, CareerDisplayComponent_For_4_For_5_Conditional_13_For_2_Template, 6, 1, "div", null, _forTrack03);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const M_r1 = \u0275\u0275nextContext().$implicit;
...
function CareerDisplayComponent_For_4_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(3);
    \u0275\u0275elementEnd()();
    \u0275\u0275repeaterCreate(4, CareerDisplayComponent_For_4_For_5_Template, 15, 14, "div", 5, _forTrack13);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const P_r18 = ctx.$implicit;
...
var CareerDisplayComponent = class _CareerDisplayComponent {
  constructor() {
    this.careerAndActivities = input();
    this.career = input();
  }
  openImage(image) {
    window.open(image);
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _CareerDisplayComponent, selectors: [["app-career-display"]], inputs: { careerAndActivities: [1, "careerAndActivities"], career: [1, "career"] }, decls: 5, vars: 1, consts: [[1, "cardBackground"], [1, "subdivisionTitle"], [1, "listStyle"], [1, "labelTitlePadding"], [1, "propertyStyle"], [1, "statementStyle"], [3, "ngStyle"], [1, "displayLayout"], [1, "warningDeprecated"], [1, "qualifierStyle"], [1, "calendarStyle"], [1, "internalLink", 3, "routerLink"], ["color", "primary", 3, "inline"], ["target", "_blank", 3, "href"], [1, "cover", 3, "click", "src"], [1, "description-text"], [1, "qualifierProperty"], [3, "click", "src"], [1, "referenceStyle"], [1, "inlineStyle"], [1, "referenceProperty"]], template: function CareerDisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275elementStart(0, "mat-card-content", 0)(1, "div", 1);
        \u0275\u0275text(2);
        \u0275\u0275elementEnd();
        \u0275\u0275repeaterCreate(3, CareerDisplayComponent_For_4_Template, 6, 1, "div", 2, _forTrack03);
        \u0275\u0275elementEnd();
      }
      if (rf & 2) {
        \u0275\u0275advance(2);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(CareerDisplayComponent, { className: "CareerDisplayComponent", filePath: "src/app/display/career-display/career-display.component.ts", lineNumber: 16 });
})();

// src/app/display/main-display/main-display.component.ts
var _forTrack04 = ($index, $item) => $item.label;
var _forTrack14 = ($index, $item) => $item.mainsnak;
...
function MainDisplayComponent_For_6_For_5_Conditional_13_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span");
    \u0275\u0275repeaterCreate(1, MainDisplayComponent_For_6_For_5_Conditional_13_For_2_Template, 6, 1, "div", null, _forTrack04);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const M_r1 = \u0275\u0275nextContext().$implicit;
...
function MainDisplayComponent_For_6_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(3);
    \u0275\u0275elementEnd()();
    \u0275\u0275repeaterCreate(4, MainDisplayComponent_For_6_For_5_Template, 15, 14, "div", 6, _forTrack14);
    \u0275\u0275elementEnd();
...
var MainDisplayComponent = class _MainDisplayComponent {
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _MainDisplayComponent, selectors: [["app-main-display"]], inputs: { mainList: [1, "mainList"], mainTitle: [1, "mainTitle"], list: [1, "list"] }, decls: 7, vars: 1, consts: [[1, "cardBackground"], [1, "subTitlePadding"], [1, "divisionTitle"], [1, "listStyle"], [1, "labelTitlePadding"], [1, "propertyStyle"], [1, "statementStyle"], [3, "ngStyle"], [1, "displayLayout"], [1, "warningDeprecated"], [1, "qualifierStyle"], [1, "calendarStyle"], [1, "internalLink", 3, "routerLink"], ["color", "primary", 3, "inline"], ["target", "_blank", 3, "href"], [1, "cover", 3, "click", "src"], [1, "description-text"], [1, "qualifierProperty"], [3, "click", "src"], [1, "referenceStyle"], [1, "inlineStyle"], [1, "referenceProperty"]], template: function MainDisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275elementStart(0, "mat-card-content", 0)(1, "div", 1)(2, "span", 2);
        \u0275\u0275text(3);
        \u0275\u0275elementEnd()();
        \u0275\u0275element(4, "router-outlet");
        \u0275\u0275repeaterCreate(5, MainDisplayComponent_For_6_Template, 8, 1, null, null, _forTrack04);
        \u0275\u0275elementEnd();
      }
      if (rf & 2) {
        \u0275\u0275advance(3);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(MainDisplayComponent, { className: "MainDisplayComponent", filePath: "src/app/display/main-display/main-display.component.ts", lineNumber: 14 });
})();
...
var EducationDisplayComponent = class _EducationDisplayComponent {
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _EducationDisplayComponent, selectors: [["app-education-display"]], inputs: { education: [1, "education"], training: [1, "training"] }, decls: 5, vars: 1, consts: [[1, "cardBackground"], [1, "subdivisionTitle"], [1, "listStyle"], [1, "labelTitlePadding"], [1, "propertyStyle"], [1, "statementStyle"], [3, "ngStyle"], [1, "displayLayout"], [1, "warningDeprecated"], [1, "qualifierStyle"], [1, "calendarStyle"], [1, "internalLink", 3, "routerLink"], ["color", "primary", 3, "inline"], ["target", "_blank", 3, "href"], [1, "cover", 3, "click", "src"], [1, "description-text"], [1, "qualifierProperty"], [3, "click", "src"], [1, "referenceStyle"], [1, "inlineStyle"], [1, "referenceProperty"]], template: function EducationDisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275elementStart(0, "mat-card-content", 0)(1, "div", 1);
        \u0275\u0275text(2);
        \u0275\u0275elementEnd();
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(EducationDisplayComponent, { className: "EducationDisplayComponent", filePath: "src/app/display/education-display/education-display.component.ts", lineNumber: 14 });
})();

// src/app/display/sociability-display/sociability-display.component.ts
var _forTrack05 = ($index, $item) => $item.label;
var _forTrack15 = ($index, $item) => $item.mainsnak;
...
function SociabilityDisplayComponent_For_4_For_5_Conditional_13_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span");
    \u0275\u0275repeaterCreate(1, SociabilityDisplayComponent_For_4_For_5_Conditional_13_For_2_Template, 6, 1, "div", null, _forTrack05);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const M_r1 = \u0275\u0275nextContext().$implicit;
...
function SociabilityDisplayComponent_For_4_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(3);
    \u0275\u0275elementEnd()();
    \u0275\u0275repeaterCreate(4, SociabilityDisplayComponent_For_4_For_5_Template, 15, 14, "div", 5, _forTrack15);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const P_r18 = ctx.$implicit;
...
var SociabilityDisplayComponent = class _SociabilityDisplayComponent {
  constructor() {
    this.sociabilityAndCulture = input();
    this.sociability = input();
  }
  openImage(image) {
    window.open(image);
  }
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _SociabilityDisplayComponent, selectors: [["app-sociability-display"]], inputs: { sociabilityAndCulture: [1, "sociabilityAndCulture"], sociability: [1, "sociability"] }, decls: 5, vars: 1, consts: [[1, "cardBackground"], [1, "subdivisionTitle"], [1, "listStyle"], [1, "labelTitlePadding"], [1, "propertyStyle"], [1, "statementStyle"], [3, "ngStyle"], [1, "displayLayout"], [1, "warningDeprecated"], [1, "qualifierStyle"], [1, "calendarStyle"], [1, "internalLink", 3, "routerLink"], ["color", "primary", 3, "inline"], ["target", "_blank", 3, "href"], [1, "cover", 3, "click", "src"], [1, "description-text"], [1, "qualifierProperty"], [3, "click", "src"], [1, "referenceStyle"], [1, "inlineStyle"], [1, "referenceProperty"]], template: function SociabilityDisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275elementStart(0, "mat-card-content", 0)(1, "div", 1);
        \u0275\u0275text(2);
        \u0275\u0275elementEnd();
        \u0275\u0275repeaterCreate(3, SociabilityDisplayComponent_For_4_Template, 6, 1, "div", 2, _forTrack05);
        \u0275\u0275elementEnd();
      }
      if (rf & 2) {
        \u0275\u0275advance(2);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SociabilityDisplayComponent, { className: "SociabilityDisplayComponent", filePath: "src/app/display/sociability-display/sociability-display.component.ts", lineNumber: 14 });
})();

// src/app/display/sources-display/sources-display.component.ts
var _forTrack06 = ($index, $item) => $item.label;
var _forTrack16 = ($index, $item) => $item.mainsnak;
...
function SourcesDisplayComponent_For_4_For_5_Conditional_13_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span");
    \u0275\u0275repeaterCreate(1, SourcesDisplayComponent_For_4_For_5_Conditional_13_For_2_Template, 6, 1, "div", null, _forTrack06);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const M_r1 = \u0275\u0275nextContext().$implicit;
...
function SourcesDisplayComponent_For_4_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(3);
    \u0275\u0275elementEnd()();
    \u0275\u0275repeaterCreate(4, SourcesDisplayComponent_For_4_For_5_Template, 15, 14, "div", 5, _forTrack16);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const P_r18 = ctx.$implicit;
...
var SourcesDisplayComponent = class _SourcesDisplayComponent {
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _SourcesDisplayComponent, selectors: [["app-sources-display"]], inputs: { sourcesList: [1, "sourcesList"], sources: [1, "sources"] }, decls: 5, vars: 1, consts: [[1, "cardBackground"], [1, "subdivisionTitle"], [1, "listStyle"], [1, "labelTitlePadding"], [1, "propertyStyle"], [1, "statementStyle"], [3, "ngStyle"], [1, "displayLayout"], [1, "warningDeprecated"], [1, "qualifierStyle"], [1, "calendarStyle"], [1, "internalLink", 3, "routerLink"], ["color", "primary", 3, "inline"], ["target", "_blank", 3, "href"], [1, "cover", 3, "click", "src"], [1, "description-text"], [1, "qualifierProperty"], [3, "click", "src"], [1, "referenceStyle"], [1, "inlineStyle"], [1, "referenceProperty"]], template: function SourcesDisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275elementStart(0, "mat-card-content", 0)(1, "div", 1);
        \u0275\u0275text(2);
        \u0275\u0275elementEnd();
        \u0275\u0275repeaterCreate(3, SourcesDisplayComponent_For_4_Template, 6, 1, "div", 2, _forTrack06);
        \u0275\u0275elementEnd();
      }
      if (rf & 2) {
        \u0275\u0275advance(2);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SourcesDisplayComponent, { className: "SourcesDisplayComponent", filePath: "src/app/display/sources-display/sources-display.component.ts", lineNumber: 15 });
})();
...
function Sparql0DisplayComponent_Conditional_5_Template(rf, ctx) {
  if (rf & 1) {
    const _r2 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "div", 5)(1, "mat-form-field", 8)(2, "input", 9);
...
function Sparql0DisplayComponent_For_7_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "label");
    \u0275\u0275text(1);
    \u0275\u0275elementStart(2, "mat-icon-button", 11)(3, "mat-icon", 7);
    \u0275\u0275text(4, "open_in_new");
    \u0275\u0275elementEnd()();
    \u0275\u0275text(5, "\xA0\xA0\xA0 ");
...
var Sparql0DisplayComponent = class _Sparql0DisplayComponent {
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _Sparql0DisplayComponent, selectors: [["app-sparql0-display"]], inputs: { sparqlSubject: [1, "sparqlSubject"], sparqlData: [1, "sparqlData"] }, features: [\u0275\u0275NgOnChangesFeature], decls: 13, vars: 5, consts: [["content", ""], [3, "ngClass"], ["appearance", "outlined", 1, "mat-elevation-z12"], [1, "cardBackground"], [1, "subTitlePadding", 3, "ngClass"], [1, "searchFormPadding"], ["mat-icon-button", "", "color", "primary", 1, "internalLink", 3, "click"], ["color", "primary", 3, "inline"], ["appearance", "outline"], ["matInput", "", "placeholder", "Search", 3, "keyup"], [1, "sparql-div", "statementStyle"], [1, "internalLink", 3, "routerLink"]], template: function Sparql0DisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        const _r1 = \u0275\u0275getCurrentView();
        \u0275\u0275elementStart(0, "div", 1)(1, "mat-card", 2)(2, "mat-card-content", 3)(3, "div", 4);
        \u0275\u0275text(4);
        \u0275\u0275elementEnd();
        \u0275\u0275template(5, Sparql0DisplayComponent_Conditional_5_Template, 5, 1, "div", 5);
        \u0275\u0275repeaterCreate(6, Sparql0DisplayComponent_For_7_Template, 3, 1, null, null, \u0275\u0275repeaterTrackByIdentity);
        \u0275\u0275elementStart(8, "button", 6, 0);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(Sparql0DisplayComponent, { className: "Sparql0DisplayComponent", filePath: "src/app/display/sparql0-display/sparql0-display.component.ts", lineNumber: 21 });
})();

// src/app/display/sparql1-display/sparql1-display.component.ts
var _forTrack07 = ($index, $item) => $item.item;
var _c09 = (a0) => ["/item", a0];
...
function Sparql1DisplayComponent_For_7_Conditional_2_Conditional_2_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275text(0);
    \u0275\u0275elementStart(1, "mat-icon-button", 12)(2, "mat-icon", 8);
...
var Sparql1DisplayComponent = class _Sparql1DisplayComponent {
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _Sparql1DisplayComponent, selectors: [["app-sparql1-display"]], inputs: { sparqlSubject: [1, "sparqlSubject"], sparqlData: [1, "sparqlData"] }, features: [\u0275\u0275NgOnChangesFeature], decls: 14, vars: 5, consts: [["content", ""], [3, "ngClass"], ["appearance", "outlined", 1, "mat-elevation-z12"], [1, "cardBackground"], [1, "subTitlePadding", 3, "ngClass"], [1, "searchFormPadding"], [1, "sparql-div"], ["mat-icon-button", "", "color", "primary", 1, "internalLink", 3, "click"], ["color", "primary", 3, "inline"], ["appearance", "outline"], ["matInput", "", "placeholder", "Search", 3, "keyup"], ["target", "_blank", 3, "href"], [1, "internalLink", 3, "routerLink"]], template: function Sparql1DisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        const _r1 = \u0275\u0275getCurrentView();
        \u0275\u0275elementStart(0, "div", 1)(1, "mat-card", 2)(2, "mat-card-content", 3)(3, "div", 4);
        \u0275\u0275text(4);
        \u0275\u0275elementEnd();
        \u0275\u0275template(5, Sparql1DisplayComponent_Conditional_5_Template, 5, 1, "div", 5);
        \u0275\u0275repeaterCreate(6, Sparql1DisplayComponent_For_7_Template, 4, 1, "div", 6, _forTrack07);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(Sparql1DisplayComponent, { className: "Sparql1DisplayComponent", filePath: "src/app/display/sparql1-display/sparql1-display.component.ts", lineNumber: 21 });
})();
...
var Sparql2DisplayComponent = class _Sparql2DisplayComponent {
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _Sparql2DisplayComponent, selectors: [["app-sparql2-display"]], inputs: { sparqlSubject: [1, "sparqlSubject"], sparqlData: [1, "sparqlData"] }, features: [\u0275\u0275NgOnChangesFeature], decls: 14, vars: 5, consts: [["content", ""], [3, "ngClass"], ["appearance", "outlined", 1, "mat-elevation-z12"], [1, "cardBackground"], [1, "subTitlePadding", 3, "ngClass"], [1, "searchFormPadding"], [1, "sparql-div", "statementStyle"], ["mat-icon-button", "", "color", "primary", 1, "internalLink", 3, "click"], ["color", "primary", 3, "inline"], ["appearance", "outline"], ["matInput", "", "placeholder", "Search", 3, "keyup"], [1, "internalLink", 3, "routerLink"]], template: function Sparql2DisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        const _r1 = \u0275\u0275getCurrentView();
        \u0275\u0275elementStart(0, "div", 1)(1, "mat-card", 2)(2, "mat-card-content", 3)(3, "div", 4);
        \u0275\u0275text(4);
        \u0275\u0275elementEnd();
        \u0275\u0275template(5, Sparql2DisplayComponent_Conditional_5_Template, 5, 1, "div", 5);
        \u0275\u0275repeaterCreate(6, Sparql2DisplayComponent_For_7_Template, 3, 1, "div", 6, \u0275\u0275repeaterTrackByIdentity);
        \u0275\u0275elementStart(8, "span")(9, "button", 7, 0);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(Sparql2DisplayComponent, { className: "Sparql2DisplayComponent", filePath: "src/app/display/sparql2-display/sparql2-display.component.ts", lineNumber: 21 });
})();
...
var Sparql3DisplayComponent = class _Sparql3DisplayComponent {
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _Sparql3DisplayComponent, selectors: [["app-sparql3-display"]], inputs: { sparqlSubject: [1, "sparqlSubject"], sparqlData: [1, "sparqlData"] }, features: [\u0275\u0275NgOnChangesFeature], decls: 14, vars: 5, consts: [["content", ""], [3, "ngClass"], ["appearance", "outlined", 1, "mat-elevation-z12"], [1, "cardBackground"], [1, "subTitlePadding", 3, "ngClass"], [1, "searchFormPadding"], [1, "sparql-div", "statementStyle"], ["mat-icon-button", "", "color", "primary", 1, "internalLink", 3, "click"], ["color", "primary", 3, "inline"], ["appearance", "outline"], ["matInput", "", "placeholder", "Search", 3, "keyup"], [1, "internalLink", 3, "routerLink"]], template: function Sparql3DisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        const _r1 = \u0275\u0275getCurrentView();
        \u0275\u0275elementStart(0, "div", 1)(1, "mat-card", 2)(2, "mat-card-content", 3)(3, "div", 4);
        \u0275\u0275text(4);
        \u0275\u0275elementEnd();
        \u0275\u0275template(5, Sparql3DisplayComponent_Conditional_5_Template, 5, 1, "div", 5);
        \u0275\u0275repeaterCreate(6, Sparql3DisplayComponent_For_7_Template, 3, 1, "div", 6, \u0275\u0275repeaterTrackByIdentity);
        \u0275\u0275elementStart(8, "span")(9, "button", 7, 0);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(Sparql3DisplayComponent, { className: "Sparql3DisplayComponent", filePath: "src/app/display/sparql3-display/sparql3-display.component.ts", lineNumber: 22 });
})();
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(Sparql4DisplayComponent, { className: "Sparql4DisplayComponent", filePath: "src/app/display/sparql4-display/sparql4-display.component.ts", lineNumber: 21 });
})();
...
var IframesDisplayComponent = class _IframesDisplayComponent {
  constructor() {
    ...
    this.urlSafe8 = input();
    this.urlSafe9 = input();
    this.urlSafe10 = input();
    this.urlSafe11 = input();
    this.urlSafe12 = input();
    this.urlSafe13 = input();
    this.urlSafe14 = input();
    this.urlSafe15 = input();
  }
  static {
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _IframesDisplayComponent, selectors: [["app-iframes-display"]], inputs: { claims: [1, "claims"], urlSafe1: [1, "urlSafe1"], urlSafe2: [1, "urlSafe2"], urlSafe3: [1, "urlSafe3"], urlSafe4: [1, "urlSafe4"], urlSafe5: [1, "urlSafe5"], urlSafe6: [1, "urlSafe6"], urlSafe7: [1, "urlSafe7"], urlSafe8: [1, "urlSafe8"], urlSafe9: [1, "urlSafe9"], urlSafe10: [1, "urlSafe10"], urlSafe11: [1, "urlSafe11"], urlSafe12: [1, "urlSafe12"], urlSafe13: [1, "urlSafe13"], urlSafe14: [1, "urlSafe14"], urlSafe15: [1, "urlSafe15"] }, decls: 6, vars: 5, consts: [[1, "cardBackground"], [1, "iframeTitlePadding"], [1, "propertyStyle"], [2, "width", "100%", "height", "100vh", "border", "none", 3, "src"], [1, "iFrameComment"]], template: function IframesDisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275elementStart(0, "mat-card-content", 0);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(IframesDisplayComponent, { className: "IframesDisplayComponent", filePath: "src/app/display/iframes-display/iframes-display.component.ts", lineNumber: 12 });
})();
...
var TextDisplayComponent = class _TextDisplayComponent {
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _TextDisplayComponent, selectors: [["app-text-display"]], inputs: { trans: [1, "trans"] }, features: [\u0275\u0275NgOnChangesFeature], decls: 5, vars: 3, consts: [[3, "ngClass"], [1, "cardBackground"], [1, "subdivision-title"], [3, "innerHtml"]], template: function TextDisplayComponent_Template(rf, ctx) {
      if (rf & 1) {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(TextDisplayComponent, { className: "TextDisplayComponent", filePath: "src/app/display/text-display/text-display.component.ts", lineNumber: 12 });
})();
...
var _forTrack08 = ($index, $item) => $item.picture;
...
function DisplayComponent_Conditional_9_For_46_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275listener("click", function DisplayComponent_Conditional_9_For_46_Template_img_click_2_listener() {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance(2);
    \u0275\u0275propertyInterpolate("src", picture_r6.picture, \u0275\u0275sanitizeUrl);
  }
}
...
function DisplayComponent_Conditional_9_For_90_Template(rf, ctx) {
  if (rf & 1) {
    const _r9 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "div")(1, "img", 58);
    \u0275\u0275listener("click", function DisplayComponent_Conditional_9_For_90_Template_img_click_1_listener() {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275propertyInterpolate("src", picture_r10.picture, \u0275\u0275sanitizeUrl);
  }
}
...
function DisplayComponent_Conditional_9_div_113_span_2_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(1);
    \u0275\u0275elementStart(2, "mat-icon-button", 54)(3, "mat-icon", 66);
    \u0275\u0275text(4, "open_in_new");
    \u0275\u0275elementEnd()();
    \u0275\u0275text(5, "\xA0\xA0\xA0\xA0\xA0 ");
    \u0275\u0275elementEnd();
...
function DisplayComponent_Conditional_9_div_113_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275template(2, DisplayComponent_Conditional_9_div_113_span_2_Template, 6, 5, "span", 64);
    \u0275\u0275elementEnd();
    \u0275\u0275element(3, "br");
    \u0275\u0275elementEnd();
  }
...
function DisplayComponent_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275elementEnd();
    \u0275\u0275repeaterCreate(16, DisplayComponent_Conditional_9_For_17_Template, 8, 6, "div", null, \u0275\u0275repeaterTrackByIdentity);
    \u0275\u0275elementEnd()()()();
    ...
    \u0275\u0275elementEnd();
    \u0275\u0275repeaterCreate(45, DisplayComponent_Conditional_9_For_46_Template, 3, 2, "div", 27, _forTrack08);
    \u0275\u0275elementEnd();
    ...
    \u0275\u0275repeaterCreate(89, DisplayComponent_Conditional_9_For_90_Template, 2, 1, "div", null, \u0275\u0275repeaterTrackByIdentity);
    \u0275\u0275elementEnd()();
    ...
    \u0275\u0275element(96, "hr");
    \u0275\u0275elementEnd();
    \u0275\u0275repeaterCreate(97, DisplayComponent_Conditional_9_For_98_Template, 7, 3, "div", 48, \u0275\u0275repeaterTrackByIdentity);
    \u0275\u0275elementEnd();
    ...
    \u0275\u0275element(103, "hr");
    \u0275\u0275elementEnd();
    \u0275\u0275repeaterCreate(104, DisplayComponent_Conditional_9_For_105_Template, 7, 3, "div", 48, \u0275\u0275repeaterTrackByIdentity);
    \u0275\u0275elementEnd()();
    ...
    this.subscription0 = this.route.paramMap.subscribe((params) => {
      ...
      this.subscription2 = this.data.subscribe((item) => {
        ...
        if (item !== void 0) {
          ...
          this.pictures = [];
          if (this.item[0].claims.P189 !== void 0) {
  ...
  sparqlDisplay(u) {
    ...
    if (u) {
      ...
      if (u[3] !== void 0) {
        ...
        if (u[3].osm_id) {
          this.sparqlSubject = "current address";
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(DisplayComponent, { className: "DisplayComponent", filePath: "src/app/display/display.component.ts", lineNumber: 67 });
})();
export {
  DisplayComponent
};
//# sourceMappingURL=display.component-O4AKMKR3.js.map
