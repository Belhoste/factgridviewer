...
export class ItemSparqlService {
  ...
  itemSparql(item) {  // get a list depending of the type of the item
    ...
    this.sparql2$ = forkJoin([this.masterTest, this.listTest]).pipe(switchMap(([res1, res2]) => this.selectSparql2(res1, res2, item)), startWith([undefined, undefined]));  // data for the component sparql2 : pupils and students  or class as a list

  forkJoin([this.sparql0$, this.sparql1$, this.sparql2$])    
